    ...
    for ( let M = 0; M < 20; ++M ) { // M for megacell
        for ( let t = 0; t <= side_max; ++t ) { // "row"
            ...
            for ( let n = 1; n <= r; n += 2 ) { // position within a row
                let y = c + n;
                cells[y].addLinks(cells[y - 1], cells[y + 1], cells[y + r]);

                for ( let neighbour of cells[y].getLinked() ) neighbour.addLink(cells[y]);
            }
        }

        for ( let i = 0, ii = side_max; i <= side_max; ++i, --ii ) {
            cells[edge_cells[0][i] + cell_max * M].addLink(cells[edge_cells[megacells[M].rL][ii] + cell_max * megacells[M].L]);
            cells[edge_cells[1][i] + cell_max * M].addLink(cells[edge_cells[megacells[M].rB][ii] + cell_max * megacells[M].B]);
            cells[edge_cells[2][i] + cell_max * M].addLink(cells[edge_cells[megacells[M].rR][ii] + cell_max * megacells[M].R]);
        }
    }
